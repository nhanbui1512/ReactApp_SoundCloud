...
import { addSongsToPlaylist, removeSongsFromPlaylist } from 'api/playlist'
...
const placeholder = {
  ...
  "songs": [
      {
        "thumbNail": "http://localhost:3000/uploads/images/thumbNail-1706281334109",
        "id": -1,
        "name": "Nightmare",
        "artistName": "onoken",
      },
          ...
          {playlistData.songs.map(song => 
            <div className={cx('song-info')}>
              ...
              <div className={cx('song-artist-name')}>
                <h6>{song.artistName} - {song.name}</h6>
              </div>
              {removeSongs.includes(song.id) ? 
                <button className={cx('removed')} onClick={() => CancleRemoveSong(song.id)}>Removed</button>
                : <button onClick={() => HandleRemoveSong(song.id)}>Remove</button>
              }
            </div>
          )}
        </div>
        <button disabled={saveChange}
          onClick={HandleSaveChange}
        >{saveChange ? "Saving..." : "Save" }</button>
      </div>
    </Popup>
  )
}