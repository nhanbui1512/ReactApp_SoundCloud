...
function Item({ active }) {
  ...
  return (
    ...
    >
      <div className={cx(['queue-item', 'row', ' relative'])}>
        <div className={cx('queue-item_drag-btn')}>
          <FontAwesomeIcon icon={faBars} />
        </div>

        <div className={cx(['relative', 'queue-item_img'])}>
          <img src="https://i1.sndcdn.com/artworks-000255301979-dep8nv-t50x50.jpg" alt="" />
          <div className={cx('queue-item_status-btn')}>
            <FontAwesomeIcon icon={faPlay} />
          </div>
        </div>
        <div className={cx(['col', 'queue-item_info'])}>
          <a href="/">3107</a>
          <span>W/n ft Duongg x Nâu</span>
        </div>
        <div className={cx(['relative', 'queue-item_footer'])}>
          <span className={cx('queue-item_total-time')}>3:52</span>

          <div className={cx('queue-item_group-btn')}>
            <button
              style={{
                fontSize: 12,
